...
query_params <- list(apikey = nty_apikey, q = movie_name)
full_uri <- GET(paste0(base_uri, resource), query = query_params)

# Send the HTTP Request to download the data
# Extract the content and convert it from JSON
response_content <- content(full_uri, "text")
...
reviews <- flatten(results$results)
